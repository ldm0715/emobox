...
- `src/features/search/QuickSearchWindow.tsx` 用 `useQuickSearchQuery` hook 驱动：监听 `quick-search-opened` 触发重载；**空 query 走 `searchEmojis({view:"all", sort:"recent"})`（全库最近优先，未用过的新图也可见）**，非空 query 走全库跨字段搜索（支持 `组*标签`）。`requestSeq` ref 保证快速输入 / 库变更时只有最新请求落地。
