...
- `src/app/LibrarySidebar.tsx` — 侧边栏。Phase 13 起布局从上到下固定：全部表情/最近使用/收藏 → 我的分组（标题栏右侧 🔍/＋；搜索收进按钮、点击才展开 `SearchBox`，`searchOpen` 参与过滤条件 → 收起即失效）→ 分组列表（**唯一 `flex:1; min-height:0; overflow-y:auto` 的弹性滚动区**，占满剩余高度）→ 未分组/回收站 → 快捷键（单行）/设置。固定项紧凑（导航行 28px、分隔线 4px、根 padding 6px），不要再给固定项 `flex` 或大留白。置顶走 `group.isPinned`（后端 `list_groups` ORDER BY 排前）+ 右键菜单「置顶/取消置顶」+ 名字前小图钉；折叠态搜索重置、`overflow-x:hidden` 防滚动条压窄图标行。详见 `docs/phase13-sidebar-groups-redesign.md`。
