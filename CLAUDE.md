...
- `src/App.tsx` owns the bulk of UI state: `allItems`, `currentView`, `searchQuery`, `sortOption`, `density`, `favorites`, `recentItems`, `settingsOpen`, `shortcutRegistered`, `shortcutError`, plus clipboard-collect counterparts. It wires the `image-copied` Tauri event listener to refresh `recentItems` and the `clipboard-collect-requested` listener to call `handleCollectFromClipboard`.
...
- `src/lib/tauri.ts` is the only place `invoke()` is called. Add new Tauri commands there with typed wrappers; do not call `invoke` directly from feature code.
- `src/types.ts` is the single source of shared contracts (e.g. `IndexedImage`, `RecentImageRecord`, `ScanSummary`, `ManagedImportSummary`). Field names use camelCase because the Rust side uses `#[serde(rename_all = "camelCase")]`.
- Feature folders: `features/import` (the single `ImportMenu` reused by toolbar + empty state), `features/library` (grid view), `features/search` (overlay window content). Hooks live next to the components that own them.

### Backend (`src-tauri/src/`, Rust + Tauri 2)

- `lib.rs` — entry point. Wires plugins (`clipboard-manager`, `dialog`, `global-shortcut`, `log`), managed state (`ShortcutRegistry`, `DatabaseState`, `RecentImagesState`), tray setup, `on_window_event` that intercepts `CloseRequested` on both `main` and `quick-search` and hides instead of closes, and `setup` calls `ShortcutRegistry::reconcile` to `unregister_all` any stale OS-level shortcuts on startup. Only the tray `退出` menu item calls `app.exit(0)`.
- `commands.rs` — 14 `#[tauri::command]` handlers, all registered via `tauri::generate_handler![...]` in `lib.rs`. Long-running work (scan, import, thumbnail, clipboard collect) is wrapped in `tauri::async_runtime::spawn_blocking`.
- `clipboard.rs` — write path: encodes PNG/JPEG/WebP via `image::load_from_memory_with_format`, GIF via `GifDecoder` (first frame only), and writes RGBA via `tauri-plugin-clipboard-manager`. Returns a `ClipboardCopyOutcome` describing source/clipboard format and animation status. Emit nothing from here; the command layer emits `image-copied`.
...
- `scanner.rs` — pure directory walker (`walkdir`) that returns `ScanSummary`; `scan_and_persist` writes the external-scan results to SQLite via `EmojiRepository::upsert_external_scan`.
- `thumbnail.rs` — on-demand `data:image/png;base64,...` for the frontend (no disk cache yet).
- `recent.rs` — in-memory `RecentImagesState` + JSON persistence at `app_data_dir/recent-images.json` (legacy path; the DB module also imports this on first launch). Limit 50, sorted by `lastUsedAt` desc, deduped by `path`.
- `tray.rs` — system tray with three fixed items: 打开主窗口 / 打开搜索浮层 / 退出.
- `quick_search.rs` — thin shell after Phase 5: only `WINDOW_LABEL`, `show_quick_search`, `hide_quick_search`, plus `normalize_shortcut` / `shortcut_parser_text` helpers (also used by `shortcut_registry`). All shortcut registration state moved to `ShortcutRegistry`.
...
- `database/` — opens `emobox.sqlite3` at `app_data_dir`, runs migrations from `src-tauri/migrations/*.sql`. Exposes `connect()` for repositories.
- `repositories/emoji_repository.rs` — SQL access for the `emojis` table (see migration `0001_create_emojis.sql`). Source types are `'external_directory' | 'managed_import' | 'clipboard'`. Partial unique index on `sha256` covers both managed paths.
- `services/` — `asset_service` (open-in-explorer, temp file + atomic rename, `encode_image_as_png` is the **only** PNG encode point in the codebase, `stage_file` and `stage_dynamic_image` for the two import paths) and `import_service` (`commit_staged` helper shared by `import_one` and `import_dynamic_image`; `static IMPORT_LOCK: Mutex<()>` serializes all writes; `find_managed_by_sha256` for dedup; rollback on DB failure).
...
`EmojiLibraryView` shows `allItems` for the `all` view and `recentItems` for the `recent` view. The two come from different commands and the UI must not assume the recent list is a subset of the indexed list — a recent image can still be copied even if the user has not re-imported its directory this session.
...
- Original user images are never copied, moved, renamed, or deleted by EmoBox, *except* by `import_managed_paths` and `import_dynamic_image` (clipboard collect) which deliberately copy into the managed assets directory. Don't add any other path that mutates user files.
- The main window and quick-search window both intercept `CloseRequested` and hide. Only the tray `退出` menu calls `app.exit(0)`.
- `copy_image_to_clipboard` writes a `tauri::Image` (RGBA) via the clipboard plugin, not a file path / `file://` / text. WebP is converted to a static image; GIF is reduced to its first frame and the outcome's `animationPreserved` is set to `Some(false)`.
- Recent list: max 50 entries, deduped by `path`, sorted by `lastUsedAt` desc. `useCount` increments on repeated use; on dedup the matching record is moved to the front with the bumped counter.
- All UI state that should persist across restarts goes through `ThemeProvider` / `localStorage: emobox.settings` (theme, sidebar collapse, default view, two global shortcuts). Don't introduce other persistence backends.
- The Rust side persists emoji metadata in SQLite (`emobox.sqlite3` at `app_data_dir`) and recent history used to be in `recent-images.json` (still imported on first run by `database::mod`). No cloud, no accounts, no network sync.
...
- Rust unit tests live in `#[cfg(test)]` modules (e.g. `asset_service.rs`, `import_service.rs`, `scanner.rs`, `recent.rs`, `clipboard.rs`, `shortcut_registry.rs`). No JS test suite; `npm run build` is the only TS-level check.
...
- New Tauri command? Add wrapper in `src/lib/tauri.ts`, types in `src/types.ts`, handler in `src-tauri/src/commands.rs`, register in `src-tauri/src/lib.rs::invoke_handler`. Long-running work goes through `tauri::async_runtime::spawn_blocking`.
...
- New view in the sidebar? Add to `viewTitles` in `App.tsx` and extend the `LibraryView` union in `types.ts` (existing pattern: `group:${string}` is reserved).
- New clipboard *write* behavior? `clipboard.rs` is the boundary.
...
- Implementation history and per-stage design notes: `docs/implementation-plan.md`, `docs/windows-image-clipboard.md`, `docs/system-tray-recent-usage.md`, `docs/search-overlay-global-shortcut.md`, `docs/phase5-clipboard-probe-results.md`. README has product-level overview; AGENTS.md has repo conventions.
